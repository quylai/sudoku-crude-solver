...
def recursiveSolver(grid):  
  ...
  solve_sudoku(grid)  # executing the solver
...
def analyzeSeqs(grid, *seqsOfStr):
  ...
  def nCount(grid):  # counting all cells currently filled in grid
    count = 0
    for x in grid:
      for y in x:
        if (y == 0):
  ...
  for aSeq in seqsOfStr:
    ...
    for x in aSeq:
      if (x == 'r'):
        ...
        gridProc = recursiveSolver(gridProc)
